...
默认 `analysisMode` 为 `standard`，不会生成 `advancedAnalysis`。

开启 `--advanced-analysis` 或 `--advanced` 时：

- `analysisMode` 变为 `advanced`。
- 报告会加入本地高级分析审计。
- `advancedAnalysis` 包含 `decisionTrace`、`gateAudit`、`dimensionRubric`、`evidenceAudit`、`upgradePlan` 和 `limitations`。
- 高级分析只把现有规则引擎的中间依据转成用户可读摘要。
- 高级分析不调用外部 LLM，不要求 API key，不上传原始日志。
