...
class CliTests(unittest.TestCase):
    def test_demo_default_url_uses_yisec_ai(self) -> None:
        ...
        self.assertIn("statEvidence", shared)
        self.assertIn("statProfile", shared)
        self.assertEqual(shared["statEvidence"]["id"], "supports_current_rank")
        self.assertLessEqual(len(shared["statEvidence"]["positiveSignals"]), 3)
        self.assertLessEqual(len(shared["statEvidence"]["riskSignals"]), 3)
        self.assertIn("token 和成本只说明投入强度", shared["statEvidence"]["ratingUse"])
    ...
    def test_demo_print_json_contains_cloud_report_url(self) -> None:
        ...
        self.assertEqual(payload["report"]["statEvidence"]["supportLevel"], 7)
        ...
        self.assertIn("七品统计支撑", payload["report"]["shareImagePrompt"])
